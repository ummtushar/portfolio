...
//   tushar.rotation.y += 0.01;
//   tushar.rotation.z += 0.01;
...
const clouds = new THREE.Mesh(
  new THREE.SphereGeometry(3.05, 32, 32), // Slightly larger radius than the moon
  new THREE.MeshStandardMaterial({
    map: cloudTexture,
    transparent: true, // Enable transparency
    opacity: 0.5, // Adjust opacity for desired cloud effect
  })
);
...
moon.position.z = 30;
moon.position.setX(-10);
clouds.position.copy(moon.position); // Ensure the clouds are positioned exactly on top of the moon
...
function moveCamera() {
  ...
  clouds.rotation.x += 0.01; // Ensure clouds rotate the same as moon
...
function animate() {
  ...
  clouds.rotation.x += 0.001; // Ensure clouds rotate the same as moon
